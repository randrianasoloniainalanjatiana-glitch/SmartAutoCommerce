...
const CommandeList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const response = await axios.get(`${DJANGO_API}/commandes/`);
        if (response.data) {
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    if (searchQuery) {
      filteredItems = filteredItems.filter(item =>
        ...
        String(getOrderReference(item))?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        String(item.num_facture ?? '')?.includes(searchQuery) ||
    ...
    if (sortConfig.key !== null) {
      filteredItems.sort((a, b) => {
        let aVal = sortConfig.key === 'num_facture' ? getOrderReferenceSortValue(a) : a[sortConfig.key];
        let bVal = sortConfig.key === 'num_facture' ? getOrderReferenceSortValue(b) : b[sortConfig.key];
        if (sortConfig.key === 'created_at') {
          aVal = parseOrderDate(a.created_at)?.getTime() ?? 0;
          bVal = parseOrderDate(b.created_at)?.getTime() ?? 0;
        }
  ...
  const handleCommandeClick = async (commande) => {
    ...
    try {
      const response = await axios.get(`${DJANGO_API}/commandes/${commande.id}/`);
      if (response.data) {
    ...
    // Récupérer les informations du livreur associé (si présent)
    if (commande?.id_livreur && user?.id) {
      try {
        const livreursRes = await axios.get(`${DJANGO_API}/livreurs/${user.id}/`);
        const allLivreurs = Array.isArray(livreursRes.data) ? livreursRes.data : [];
        const matched = allLivreurs.find((l) => String(l?.id) === String(commande.id_livreur));
        setLivreurInfo(matched || null);
      } catch (error) {
      ...
      <div className="max-w-6xl mx-auto bg-white dark:bg-gray-900 shadow-xl rounded-xl overflow-hidden border border-gray-200 dark:border-gray-800">
        ...
        <div className="px-6 py-5 border-b border-gray-200 dark:border-gray-800 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <input type="text" placeholder="Rechercher par client, adresse, N° facture..."
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 uppercase text-[11px] font-bold tracking-wider border-b border-gray-200 dark:border-gray-800">
                <th onClick={() => requestSort('num_facture')} className="px-6 py-4 cursor-pointer hover:text-cyan-600 select-none">N° Facture {getSortIcon('num_facture')}</th>
              ...
              {currentItems.length > 0 ? currentItems.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors group cursor-pointer" onClick={() => handleCommandeClick(item)}>
                  <td className="px-6 py-4">
                    <div className="text-sm font-bold text-gray-900 dark:text-white">{getOrderReference(item)}</div>
                    <div className="text-[10px] text-gray-400 font-medium">{formatOrderDate(item.created_at)}</div>
                  </td>
                  <td className="px-6 py-4">
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">Commande {getOrderReference(selectedCommande)}</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={handlePrintInvoicePdf}
                    className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-cyan-500 text-white text-sm font-semibold hover:bg-cyan-600 transition-colors"
                  >
                    <Printer size={16} />
                    Imprimer en PDF
                  </button>
              ...
              <div className="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-4 mb-4">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Informations Livreur</h3>
                {selectedCommande?.id_livreur && livreurInfo ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <span className="text-sm text-gray-500 dark:text-gray-400">Nom</span>
                      <p className="font-medium text-gray-900 dark:text-white">
                        {livreurInfo.prenom || ''} {livreurInfo.Nom || ''}
                      </p>
                    </div>
                    <div>
                      <span className="text-sm text-gray-500 dark:text-gray-400">Téléphone</span>
                      <p className="font-medium text-gray-900 dark:text-white">{livreurInfo.telephone || '—'}</p>
                    </div>
                    <div className="md:col-span-2">
                      <span className="text-sm text-gray-500 dark:text-gray-400">Email</span>
                      <p className="font-medium text-gray-900 dark:text-white">{livreurInfo.email || '—'}</p>
                    </div>
                  </div>
                ) : (
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    Aucun livreur n'est actuellement affecté à cette commande.
                  </p>
                )}
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-gray-700 dark:text-gray-300">
                      {formatOrderDateTime(selectedCommande.created_at)}
                    </p>
                  </div>
                </div>
              </div>
