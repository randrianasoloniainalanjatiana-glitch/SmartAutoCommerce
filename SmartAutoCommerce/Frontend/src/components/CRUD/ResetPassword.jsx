...
const ResetPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setError("");
        try {
            const response = await fetch(`${DJANGO_API}/auth/reset-password/`, {
                method: "POST",
    ...
    const handleResend = async () => {
        if (cooldown > 0) return;
        try {
            const response = await fetch(`${DJANGO_API}/auth/forgot-password/`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            <div className="w-full max-w-md bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl dark:shadow-2xl border border-gray-100 dark:border-gray-700">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        ...
                        {confirmPassword && confirmPassword === newPassword && (
                            <p className="text-xs text-green-500 mt-1 font-medium inline-flex items-center gap-1"><Check size={14} strokeWidth={2.5} /> Les mots de passe correspondent</p>
                        )}
                    </div>
