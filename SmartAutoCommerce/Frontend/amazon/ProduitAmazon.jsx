...
useEffect(() => {
  let interval = null; // ← déclaré ici pour pouvoir le nettoyer

  const fetchProduits = async () => {
    try {
      // Étape 1 : Lancer le job
      ...
      console.log("Job lancé :", job_id);

      // Étape 2 : Interroger le statut toutes les 3 secondes
      interval = setInterval(async () => {
        const statusRes = await fetch(
          `https://motivational-mica-unfailing.ngrok-free.dev/cluster/status/${job_id}`,
          { headers: { "ngrok-skip-browser-warning": "true" } }
        );
        ...
        if (statusData.finished_at !== null && statusData.result !== null) {
          clearInterval(interval);
          const tousLesProduits = Object.values(statusData.result).flat();
        ...
        if (statusData.error !== null) {
          clearInterval(interval);
          setError("Erreur serveur : " + statusData.error);
          setLoading(false);
        }
  ...
  // ← IMPORTANT : nettoie l'intervalle si le composant se démonte
  return () => {
            ...
            >
              ...
              <div className="p-4 flex flex-col flex-grow border-t border-gray-50">
                ...
                <div className="flex items-center mb-3 gap-2">
                  <StarRating note={produit.Note} size={14} />
